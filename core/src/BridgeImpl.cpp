...
namespace dm_bridge {
  ...
  BridgeImpl::BridgeImpl(Bridge::Options& opts) : loop(uvw::loop::create()) {
    ...
    if (!tcp_handle) {
      throw std::runtime_error("Failed to create TCP handle");
    }

    tcp_handle->on<uvw::close_event>([this](const auto& event, auto& handle) {
    ...
    tcp_handle->on<uvw::listen_event>([this](const auto& event, auto& handle) {
      this->on_listen_tcp_handle_listen(event, handle);
    });

    tcp_handle->on<uvw::shutdown_event>([this](const auto& event, auto& handle) {
      this->on_listen_tcp_handle_shutdown(event, handle);
    });
  ...
  void BridgeImpl::on_listen_tcp_handle_shutdown(const uvw::shutdown_event&, uvw::tcp_handle& handle) noexcept {
    this->invalid_state = true;
    close_all_handles(handle.parent());
    try {
      SPDLOG_INFO("[TCP LISTEN HANDLE] Close handle");
